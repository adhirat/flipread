export const authView = `
<!-- Auth View -->
<div id="auth-view" class="auth-container">
  <div class="auth-box">
    <div style="text-align:center;margin-bottom:32px">
      <a href="/" class="logo" style="justify-content:center;margin-bottom:12px">
        <img src="/logo.png" alt="FlipRead" style="height:32px;width:auto">
        <span>FlipRead</span>
      </a>
      <h2 style="font-size:24px" id="auth-title">Welcome Back</h2>
      <p id="auth-subtitle" style="font-size:14px;color:var(--text-secondary);margin-top:8px"></p>
    </div>
    
    <div id="auth-msg" class="msg"></div>
    
    <!-- Login/Register Fields -->
    <div id="auth-fields">
      <div class="form-group"><input id="auth-name" placeholder="Full Name" class="hidden"></div>
      <div class="form-group"><input id="auth-email" placeholder="Email Address" type="email"></div>
      <div class="form-group">
        <input id="auth-pass" placeholder="Password" type="password">
        <div id="forgot-link-container" style="text-align:right;margin-top:8px">
          <a onclick="setAuthMode('forgot')" style="font-size:12px;color:var(--accent-cyan);cursor:pointer">Forgot Password?</a>
        </div>
      </div>
    </div>

    <!-- Reset Token Field (Hidden by default) -->
    <div id="reset-fields" class="hidden">
      <div class="form-group"><input id="reset-pass" placeholder="New Password (min 8 characters)" type="password"></div>
    </div>

    <button onclick="submitAuth()" class="btn" style="width:100%" id="auth-btn">Sign In</button>
    
    <div id="auth-footer" style="text-align:center;margin-top:20px;font-size:14px;color:var(--text-secondary)">
      <span id="auth-toggle-text">New here?</span> 
      <a onclick="toggleAuthMode()" style="color:var(--accent-cyan);cursor:pointer;font-weight:600" id="auth-toggle-link">Create Account</a>
    </div>

    <div id="back-to-login" class="hidden" style="text-align:center;margin-top:20px;font-size:14px">
      <a onclick="setAuthMode('login')" style="color:var(--text-secondary);cursor:pointer">← Back to Login</a>
    </div>
  </div>
</div>
`;
