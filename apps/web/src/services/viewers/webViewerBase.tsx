...
const WebViewerHeader = ({
    safeTitle, logoUrl, storeUrl, showTTS, showNightShift, showFullMode, showZoom
}: {
    safeTitle: string; logoUrl: string; storeUrl: string;
    showTTS: boolean; showNightShift: boolean; showFullMode: boolean; showZoom: boolean;
}) => (
...
const WebFooterControls = () => (
    <div class="controls up" id="main-footer">
        <div class="footer-icons-mobile">
            <button class="header-icon" onclick="window.prevPage()" title="Previous">
            ...
            <button class="header-icon" onclick="window.nextPage()" title="Next">
...
const HighlightMenu = ({ showHighlights }: { showHighlights: boolean }) => {
...
// ─── CSS for web viewer ─────────────────────────────────────────
const getWebViewerStyles = (accent: string, bg: string, sidebarStyles: string, extraStyles: string): string => `
        :root { --accent: ${accent}; }
        ...
        .header {
            ...
            border-bottom: none; color: white; height: 60px; z-index: 1000;
        ...
        .header-left { display: flex; align-items: center; gap: 10px; pointer-events: auto; flex: 1; }
        .header-name {
            position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);
            font-size: 14px; font-weight: 600; letter-spacing: 0.5px;
            text-shadow: 0 2px 4px rgba(0, 0, 0, 0.5);
            white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
            max-width: 40vw; pointer-events: auto; text-align: center; color: #fff; z-index: 10;
        }
        .header-icons { display: flex; gap: 10px; align-items: center; justify-content: flex-end; pointer-events: auto; flex: 1; }
        .header-logo { height: 32px; width: auto; max-width: 120px; object-fit: contain; border-radius: 4px; }

        .header-icon {
            width: 32px; height: 32px; border-radius: 50%;
            background: rgba(0, 0, 0, 0.2); display: flex; align-items: center; justify-content: center;
            cursor: pointer; transition: all 0.2s ease; color: #fff; font-size: 14px;
            border: 1px solid rgba(255, 255, 255, 0.1); text-decoration: none;
        }
        .header-icon:hover { background: rgba(255, 255, 255, 0.2); transform: scale(1.05); }

        /* Footer Controls */
        .controls {
            display: none; align-items: center; justify-content: center; gap: 15px;
            padding: 10px 15px; background: rgba(20, 20, 20, 0.95);
            backdrop-filter: blur(15px); border-top: 1px solid rgba(255, 255, 255, 0.05);
        ...
        .footer-icons-mobile { display: flex; width: 100%; justify-content: space-between; align-items: center; gap: 10px; }
        .mobile-icons-center {
            display: flex; gap: 8px; align-items: center; justify-content: center;
            background: rgba(255, 255, 255, 0.05); padding: 4px 12px;
            border-radius: 30px; border: 1px solid rgba(255, 255, 255, 0.1);
        }
        .mobile-icons-center .header-icon { margin: 0; width: 32px; height: 32px; background: transparent; border: none; box-shadow: none; }
        ...
        @media (max-width: 768px) {
            ...
            .header-icons { display: flex !important; gap: 8px; }
            ...
            .header-icons .header-icon { width: 32px; height: 32px; font-size: 14px; }
            .header-logo { display: block !important; height: 24px !important; width: 24px !important; }
            .controls {
                display: flex !important; bottom: 15px !important; left: 50% !important;
                transform: translate(-50%, 0) !important; width: calc(100% - 32px) !important;
                max-width: 500px !important; background: none !important; border: none !important;
            ...
            .footer-icons-mobile {
                display: flex !important; width: 100%; padding: 8px 20px;
                ...
                pointer-events: auto !important; justify-content: space-between; align-items: center;
            }
            .header-left .header-name { display: none; }
            .header-name { max-width: 50vw; font-size: 13px; color: #fff; text-align: left; }
            #set-m { 
        ...
        @media (min-width: 769px) and (max-width: 1024px) {
            .header-name {
                ...
                margin-left: 20px !important; flex: 2 !important; max-width: none !important;
            }
        ...
        @media (max-width: 768px) { #scroll-top { bottom: 80px; right: 20px; width: 40px; height: 40px; } }
...
// ─── Client-side JS (stays as template string) ──────────────────
const getWebViewerScript = (fileUrl: string, safeTitle: string, storeName: string, showHighlights: boolean, extraScripts: string): string => `
...
export function getWebViewerBase(options: WebViewerOptions): string {
    const { 
        title, fileUrl, coverUrl, settings, showBranding, 
        logoUrl = '', storeUrl = '/', 
        extraStyles = '', extraHtml = '', extraScripts = '', 
        settingsHtml = '', dependencies = [], 
        showTTS = false, storeName = 'SHOPUBLISH', 
        showHighlights = true, showZoom = false,
        showNightShift = false, showTexture = false, showFullMode = false
    } = options;
    ...
    const sidebarStyles = getSidebarStyles(accent);
    const sidebarHtml = getSidebarHtml(showHighlights);
    const viewerStyles = getWebViewerStyles(accent, bg, sidebarStyles, extraStyles);
    const viewerScript = getWebViewerScript(fileUrl, safeTitle, storeName, showHighlights, extraScripts);
    ...
    const page = (
        ...
        <body class="up">
            ...
            <HighlightMenu showHighlights={!!showHighlights} />
            ...
            {/* Settings Panel (viewer-specific) */}
