    ...
    <div id="composer-md" class="composer-pane card" style="display:none;margin-top:24px;padding:0;overflow:hidden;border:1px solid var(--border)">
        ...
        <div class="editor-toolbar" style="display:flex;flex-wrap:wrap;gap:8px;padding:12px;background:var(--bg-elevated);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:2">
            ...
            <div class="toolbar-group" style="display:flex;gap:4px;border-left:1px solid var(--border);padding-left:12px;margin-left:8px">
                <button class="btn-tool" x-on:click="window.insertMd('**','**')" title="Bold"><i class="fas fa-bold"></i></button>
                <button class="btn-tool" x-on:click="window.insertMd('*','*')" title="Italic"><i class="fas fa-italic"></i></button>
                <button class="btn-tool" x-on:click="window.insertMd('# ','')" title="H1"><i class="fas fa-heading"></i>1</button>
                <button class="btn-tool" x-on:click="window.insertMd('> ','')" title="Quote"><i class="fas fa-quote-left"></i></button>
                <button class="btn-tool" x-on:click="window.insertMd('- ','')" title="List"><i class="fas fa-list-ul"></i></button>
                <button class="btn-tool" x-on:click="window.insertMd('[Link Text](',')')" title="Link"><i class="fas fa-link"></i></button>
            </div>
            <div style="flex:1"></div>
            <button class="btn-outline" style="padding:6px 12px;font-size:12px;" x-on:click="window.downloadMd()"><i class="fas fa-download"></i> Export .md</button>
        </div>
        ...
        <div style="display:flex;height:700px;background:var(--bg-secondary)">
            ...
            <div style="flex:1;border-right:1px solid var(--border);display:flex;flex-direction:column;background:#fff">
                <textarea id="md-editor-textarea" x-on:input="window.updateMdPreview()" style="flex:1;width:100%;border:none;padding:24px;font-family:ui-monospace,monospace;font-size:14px;line-height:1.6;outline:none;resize:none" placeholder="# Welcome to Shopublish Markdown..."></textarea>
            </div>
