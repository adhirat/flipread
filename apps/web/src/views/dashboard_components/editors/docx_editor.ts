...
<div id="composer-doc" class="composer-pane card" style="margin-top:24px;padding:0;overflow:hidden;border:1px solid var(--border)">
    ...
    <div class="editor-toolbar" style="display:flex;flex-wrap:wrap;gap:8px;padding:12px;background:var(--bg-elevated);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:2">
        <div class="toolbar-group" style="display:flex;gap:4px;border-right:1px solid var(--border);padding-right:8px">
            ...
            <button class="btn-tool" onclick="execCmd('underline')" title="Underline"><i class="fas fa-underline"></i></button>
            <button class="btn-tool" onclick="execCmd('strikeThrough')" title="Strike"><i class="fas fa-strikethrough"></i></button>
        </div>
        <div class="toolbar-group" style="display:flex;gap:4px;border-right:1px solid var(--border);padding-right:8px">
            <button class="btn-tool" onclick="execCmd('formatBlock','H1')" title="Heading 1"><i class="fas fa-heading"></i>1</button>
            <button class="btn-tool" onclick="execCmd('formatBlock','H2')" title="Heading 2"><i class="fas fa-heading"></i>2</button>
            <button class="btn-tool" onclick="execCmd('formatBlock','P')" title="Paragraph"><i class="fas fa-paragraph"></i></button>
        </div>
        <div class="toolbar-group" style="display:flex;gap:4px;border-right:1px solid var(--border);padding-right:8px">
            <button class="btn-tool" onclick="execCmd('insertUnorderedList')" title="Bullet List"><i class="fas fa-list-ul"></i></button>
            <button class="btn-tool" onclick="execCmd('insertOrderedList')" title="Numbered List"><i class="fas fa-list-ol"></i></button>
        </div>
        <div class="toolbar-group" style="display:flex;gap:4px;border-right:1px solid var(--border);padding-right:8px">
            <button class="btn-tool" onclick="execCmd('justifyLeft')" title="Align Left"><i class="fas fa-align-left"></i></button>
            <button class="btn-tool" onclick="execCmd('justifyCenter')" title="Align Center"><i class="fas fa-align-center"></i></button>
            <button class="btn-tool" onclick="execCmd('justifyRight')" title="Align Right"><i class="fas fa-align-right"></i></button>
        </div>
        <div class="toolbar-group" style="display:flex;gap:4px">
            <button class="btn-tool" onclick="insertLinkPrompt()" title="Link"><i class="fas fa-link"></i></button>
            <button class="btn-tool" onclick="insertImagePrompt()" title="Image"><i class="fas fa-image"></i></button>
            <button class="btn-tool" onclick="execCmd('removeFormat')" title="Clear formatting"><i class="fas fa-eraser"></i></button>
        </div>
        <div style="flex:1"></div>
        <button class="btn" style="padding:6px 12px;font-size:12px;" onclick="generateDocx()"><i class="fas fa-download"></i> Export DOCX</button>
    </div>
