...
export const docsView = () => (
    <>
        ...
        <div id="view-docs" class="view-section">
          ...
          <div class="books-tabs">
            <button class="books-tab active" id="tab-my" x-on:click="window.switchBooksTab('my')">My Docs</button>
            <button class="books-tab" id="tab-shared" x-on:click="window.switchBooksTab('shared')">Shared with Me</button>
          </div>

          <div id="my-books-content">
            <div class="upload-zone" x-on:click="document.getElementById('file-input').click()">
              ...
              <p style="color:var(--text-muted);font-size:12px;margin-top:4px">PDF, EPUB, DOCX, ODT, PPTX, ODP, XLSX, ODS, CSV, TSV, TXT, MD, HTML, Images</p>
              <div id="upload-msg" class="msg" style="margin-top:16px;display:inline-block"></div>
            </div>
            <input type="file" id="file-input" accept=".pdf,.epub,.doc,.docx,.odt,.ppt,.pptx,.odp,.xls,.xlsx,.ods,.csv,.tsv,.txt,.md,.rtf,.html,.htm,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp3,.wav,.ogg,.m4a,.mp4,.webm,.mov,.avi" style="display:none" x-on:change="window.uploadBook(event)" multiple />

            <div class="books-toolbar">
              <div class="books-search-wrap">
                <i class="fas fa-search"></i>
                <input type="text" class="books-search" id="books-search-input" placeholder="Search docs..." x-on:input="window.filterBooks()" />
              </div>
              <select id="books-type-filter" x-on:change="window.filterBooks()" style="padding:10px 16px;border-radius:12px;border:1px solid var(--border);background:var(--bg-elevated);color:var(--text-primary);font-family:'Work Sans',sans-serif;cursor:pointer">
              ...
              <select id="books-sort" x-on:change="window.filterBooks()" style="padding:10px 16px;border-radius:12px;border:1px solid var(--border);background:var(--bg-elevated);color:var(--text-primary);font-family:'Work Sans',sans-serif;cursor:pointer">
              ...
              <div class="view-toggle">
                <button id="bv-grid" class="active" x-on:click="window.setBookView('grid')" title="Grid View"><i class="fas fa-th"></i></button>
                <button id="bv-list" x-on:click="window.setBookView('list')" title="List View"><i class="fas fa-list"></i></button>
              </div>
            </div>
    ...
    <div id="edit-modal" class="modal">
      <div class="modal-content">
        <div class="close-btn" x-on:click="window.hideModal('edit-modal')">&times;</div>
        ...
        <div class="form-group">
          ...
          <p style="font-size:12px;color:var(--text-muted);margin-top:4px">
            <b>Setup:</b> Point your domain's <b>CNAME</b> record to <code>SHOPUBLISH.COM</code>.
          </p>
        </div>
        <div class="form-group">
        ...
        <div id="edit-album-container" style="display:none; margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--border);">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
            ...
            <button id="add-album-file-btn" class="btn-outline" style="padding: 4px 12px; font-size: 12px;" x-on:click="document.getElementById('album-file-input').click()">Add Files</button>
            <input type="file" id="album-file-input" style="display:none" multiple x-on:change="window.addAlbumFiles(event)" />
          </div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:24px;">
          <button x-on:click="window.hideModal('edit-modal')" class="btn-outline" style="border:none">Cancel</button>
          <button x-on:click="window.saveBook()" class="btn">Save Changes</button>
        </div>
      </div>
    </div>
    ...
    <div id="share-modal" class="modal">
      <div class="modal-content">
        <div class="close-btn" x-on:click="window.hideModal('share-modal')">&times;</div>
        ...
        <div style="display:flex;gap:8px;margin-bottom:8px">
          ...
          <button x-on:click="window.shareBook()" class="btn" style="padding:12px 20px">Share</button>
        </div>
